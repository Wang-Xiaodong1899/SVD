...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="/home/wxd/video-style-transfer/FateZero/ckpt/stable-diffusion-v1-4",
        required=True,
...
def main():
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            ...
            for i in range(len(models)):
                ...
                load_model = UNetSpatioTemporalConditionModel(cross_attention_dim=768, in_channels=4)
                inner_tensors = {}
                with safe_open("/home/wxd/video-generation/diffusers/examples/text_to_image/sd-drive-ep40/unet/diffusion_pytorch_model.safetensors", framework="pt", device='cpu') as f:
                    for k in f.keys():
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                model_pred = unet(noisy_latents, timesteps, encoder_hidden_states, added_time_ids, action=action, image_context=image_context).sample
    ...
    if accelerator.is_main_process:
        ...
        # os.makedirs(os.path.join(args.output_dir, 'pure_unet'),exist_ok=True)
        # save_file(unet.state_dict(), os.path.join(args.output_dir, 'pure_unet', "diffusion_pytorch_model.fp16.safetensors"))
