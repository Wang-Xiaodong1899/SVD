...
def parse_args():
    ...
    parser.add_argument("--noise_offset", type=float, default=0.7, help="The scale of noise offset.")
    parser.add_argument(
        "--validation_epochs",
        type=int,
        default=5,
...
def main():
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        image_encoder = CLIPVisionModelWithProjection.from_pretrained(
            '/ssd_datasets/wxiaodong/ckpt/stable-video-diffusion-img2vid-xt', subfolder="image_encoder", revision=args.revision, variant=args.variant
        )
        feature_extractor = CLIPImageProcessor.from_pretrained('/ssd_datasets/wxiaodong/ckpt/stable-video-diffusion-img2vid-xt', subfolder="feature_extractor")
    ...
    for k, v in tensors.items():
        if 'attn2' in k: # skip cross-attention layers
            pass
        elif 'mid_block' in k and 'resnets' in k:
            strs = k.split('.')
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # steers = batch['steer'].to(weight_dtype)
                # speeds = batch['speed'].to(weight_dtype)
                ...
                first_frames = video_frames[:, 0]
                # add noise
                noise_prob = 0.1
                prob = torch.rand(1).item()
                if prob < noise_prob:
                    noise_aug_strength = 0.02
                    noise_c = torch.randn(first_frames.shape, device=first_frames.device)
                    first_frames = first_frames + noise_aug_strength * noise_c

                condition_frame = _resize_with_antialiasing(first_frames.to(torch.float32).cpu(), (224, 224))
                condition_frame = (condition_frame + 1.0) / 2.0

                condition_frame = feature_extractor(
                                images=condition_frame,
                                do_normalize=True,
                ...
                condition_frame = condition_frame.to(accelerator.device, dtype=weight_dtype)

                image_embeddings = image_encoder(condition_frame).image_embeds
                image_embeddings = image_embeddings[:, None] # b 1 1024
                ...
                latents_context = vae.encode(first_frames.to(weight_dtype)).latent_dist.sample() # b c h w
                latents_context = latents_context * vae.config.scaling_factor

                latents_context = repeat(latents_context, 'b c h w -> b l c h w', l=l)

                # Sample noise that we'll add to the latents
                noise = torch.randn_like(latents)
                if args.noise_offset: # 0.7
                ...
                added_time_ids = get_add_time_ids(bsz).to(latents.device)

                noisy_latents = torch.cat([noisy_latents, latents_context], dim=2)
