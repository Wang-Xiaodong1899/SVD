...
class CombinedZipFileSystem(fsspec.archive.AbstractArchiveFileSystem):
    ...
    """This file system opens several ZIP blobs from a given file system and
    provide the file access inside the ZIP blobs. It is forkable and compatible
    with multi worker data loader of the PyTorch.

    Args:
        fs (fsspec.AbstractFileSystem): The file system to open the ZIP blobs.
        paths (list): The ZIP path list to open.
    ...
    def __init__(
        self, fs: fsspec.AbstractFileSystem, paths: list,
        ...
        for path in paths:
            ...
            if enable_cached_info:
                ...
                for filename, i in infodict.items():
                    self._belongs_to[filename] = path

                self._info[path] = infodict

            else:
                with fs.open(path) as f:
                    with zipfile.ZipFile(f) as zf:
                        infodict = self._info[path] = {}
                        for i in zf.infolist():
                            self._belongs_to[i.filename] = path
                            infodict[i.filename] = [
    ...
    def _get_dirs(self):
        if self.dir_cache is None:
            ...
            for file_name, zip_path in self._belongs_to.items():
    ...
    def _open(
        ...
        header_offset, _, is_dir = self._info[zip_path][path]
        if is_dir:
            raise FileNotFoundError(path)
        ...
        if method == zipfile.ZIP_STORED:
            return common.PartialReadableRawIO(f, offset, offset + size)
