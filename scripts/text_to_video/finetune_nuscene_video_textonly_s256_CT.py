...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="text2video_textonly_s256_CT",
        help=(
...
def main():
    ...
    miss_keys, ignore_keys = unet.load_state_dict(tensors, strict=False)
    if accelerator.is_main_process:
        print('load weights to continue training...')
        print('miss_keys: ', miss_keys)
        print('ignore_keys: ', ignore_keys)

    # find learnable parameters
    # image tensors
    image_unet_dir = os.path.join('/ssd_datasets/wxiaodong/ckpt/drive-s256-ep40', 'unet')
    image_unet_files = os.listdir(image_unet_dir)
    if 'diffusion_pytorch_model.safetensors' in image_unet_files:
        tmp_tensors = {}
        with safe_open(os.path.join(image_unet_dir, "diffusion_pytorch_model.safetensors"), framework="pt", device='cpu') as f:
            for k in f.keys():
                tmp_tensors[k] = f.get_tensor(k)
    else:
        # diffusion_pytorch_model.bin
        tmp_tensors = torch.load(os.path.join(image_unet_dir, 'diffusion_pytorch_model.bin'), map_location=torch.device('cpu'))

    new_state_dicts = OrderedDict()
    for k, v in tmp_tensors.items():
        if 'mid_block' in k and 'resnets' in k:
            ...
            new_state_dicts[k] = v
    miss_keys = set(unet.state_dict().keys()) - set(new_state_dicts.keys())
    if accelerator.is_main_process:
        print('trainable keys: ', miss_keys)
