...
# NOTE
# Drop means
# We should 10% drop text, image-context
# We use unet_action_v1, We delete the added_time_ids
...
from diffusers.models.unet_action_v1 import UNetSpatioTemporalConditionModel_Action
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="text2video_textonly_s256",
        help=(
    ...
    parser.add_argument(
        "--drop_text", action='store_true', default=True, help="Whether or not drop text caption."
    )
    parser.add_argument(
        "--drop_image", action='store_true', default=True, help="Whether or not drop image."
    )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if args.drop_image:
                    ...
                    if prob < image_drop_prob:
                        image_context = torch.zeros_like(latents[:, 0]).to(weight_dtype).to(latents.device)
                    else:
                ...
                model_pred = unet(noisy_latents, timesteps, encoder_hidden_states, image_context=image_context).sample
