...
def load_models(pretrained_model_name_or_path = '/mnt/lustrenew/wangxiaodong/smodels-vis/action-v30--5e-5/checkpoint-6000', device='cuda:0'):
    ...
    scheduler = DDIMScheduler.from_pretrained('/mnt/lustrenew/wangxiaodong/smodels/image-v-s768-1e-4/checkpoint-8000', subfolder="scheduler")
...
def main(
    ...
    train_dataset = Actionframes(None, split='val', tokenizer=tokenizer, img_size=(384, 768), max_video_len = num_frames)
    ...
    #     if 'scene-0092' in scenes or 'scene-0920' in scenes or 'scene-0926' in scenes or 'scene-0330' in scenes:
    #         new_batch.append({k:v for k, v in batch.items()})
    #     else:
            # continue
    ...
    for n, batch in tqdm(enumerate(train_dataloader)):
        ...
        # guide_image = Image.open('/mnt/cache/wangxiaodong/SDM/scripts/action_to_video/metrics/scene-0555.jpg').convert('RGB')

        video = pipeline(batch, num_frames=train_frames, height=384, width=768).frames
        ...
        for r in range(1, roll_out):
            ...
            video_ex = pipeline(curr_batch, image=[curr_frame], prompt=[curr_prompt], num_frames=train_frames, height=384, width=768).frames
