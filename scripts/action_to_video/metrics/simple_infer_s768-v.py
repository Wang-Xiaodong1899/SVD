...
def load_models(pretrained_model_name_or_path = '/mnt/lustrenew/wangxiaodong/smodels-vis/video-v11-v-imclip-s768-5e-5/checkpoint-4000', device='cuda:0'):
    text_encoder = CLIPTextModel.from_pretrained(
                '/mnt/lustrenew/wangxiaodong/smodels/image-v-s768-1e-4/checkpoint-8000', subfolder="text_encoder", variant='fp16'
    )
    vae = AutoencoderKL.from_pretrained(
                '/mnt/lustrenew/wangxiaodong/smodels/image-v-s768-1e-4/checkpoint-8000', subfolder="vae", variant='fp16'
    )
    clip_model = CLIPModel.from_pretrained(
        "/mnt/lustrenew/wangxiaodong/models/CLIP-ViT-H-14-laion2B-s32B-b79K", torch_dtype=torch.float16)
    ...
    tokenizer = CLIPTokenizer.from_pretrained('/mnt/lustrenew/wangxiaodong/smodels/image-v-s768-1e-4/checkpoint-8000', subfolder="tokenizer")
    scheduler = DDPMScheduler.from_pretrained('/mnt/lustrenew/wangxiaodong/smodels/image-v-s768-1e-4/checkpoint-8000', subfolder="scheduler")
    feature_extractor = CLIPImageProcessor.from_pretrained('/mnt/lustrenew/wangxiaodong/smodels/image-v-s768-1e-4/checkpoint-8000', subfolder="feature_extractor")
...
def main(
    pretrained_model_name_or_path = '/mnt/lustrenew/wangxiaodong/smodels-vis/video-v11-v-imclip-s768-5e-5/checkpoint-4000',
    num_frames = 15,
    ...
    for idx, item in tqdm(enumerate(meta_data)):
        ...
        video = pipeline(image, num_frames=train_frames, prompt=prompt, action=None, height=384, width=768).frames[0]
        ...
        for t in range(roll_out):
            ...
            video_2 = pipeline(last_frame, num_frames=train_frames, prompt=prompt, action=None, height=384, width=768).frames[0]
