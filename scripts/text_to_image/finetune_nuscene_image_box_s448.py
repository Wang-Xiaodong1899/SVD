...
def main():
    ...
    print(f'prediction_type: {noise_scheduler.config.prediction_type}')
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                latents = vae.encode(batch["pixel_values"].flatten(0, 2).to(weight_dtype)).latent_dist.sample()
                ...
                _3dbox_embeddings = layout_encoder\
                            .forward_features(_3dbox_images.flatten(0, 2).to(weight_dtype))\
                            .flatten(-2).permute(0, 2, 1)
                hdmap_images = batch["hdmap_images"].to(latents.device)
                hdmap_embeddings = layout_encoder\
                            .forward_features(hdmap_images.flatten(0, 2).to(weight_dtype))\
                            .flatten(-2).permute(0, 2, 1)
