...
from llama.attention_ngram import LlamaModeling
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="Llama_action_ngram_train",
        help=(
...
def main():
    ...
    # for action
    from action_llama_ngram_train_config import LlamaConfig
    config = LlamaConfig()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(action_model):
                ...
                ngram_labels = rearrange(ngram_labels, 'b f s d -> b f (s d)') # (b f scope 2) # bsz, 13, 8 if h8_ms16

                logits = logits[:, :ngram_labels.shape[1]]

                shift_logits = logits #bsz, 13, 8
                shift_labels = ngram_labels # bsz, 13, 8
                ...
                if args.loss == 'l2':
                    ...
                    loss_action = loss_action.mean(dim=-1)
                    # loss_action = (loss_action.view(bsz, -1) * loss_mask).mean() # NOTE original low version
                    # TODO
                    # here, we mitigate the history action predict loss, but use the total length
                    # so the loss looks like more small, because divided by the total length!!!
                    # so debug multiple training batch-size
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.eval_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        if config.eval_train_sample:
                            for n, batch in tqdm(enumerate(train_dataloader)):
                                ...
                                with torch.no_grad():
                                    ...
                                    # config.prediction_scope = 4 # NOTE use GT to predict how long
                                    print(f'predict scope {config.prediction_scope}')
                                    for ac_index in tqdm(range(0, len(action[0]-config.history_len), config.prediction_scope)):
                                        ...
                                        # NOTE debug
                                        # if we use more history GT, can we get more low accuracy?
                                        # TODO test only, if history_len=14?
                                        ...
                                        for cur_pos in tqdm(range(start_pos, total_len)):
                                            ...
                                            if n_gram > 1: # b 1 ngram*2
                                                ...
                                                # if config.prediction_scope == 1:
                                                takes = config.prediction_scope if config.prediction_scope < n_gram else n_gram
                                                prediction = prediction[:, :takes] # only take first prediction action
                                                pred_action.append(prediction)
                                            else:
                                                pred_action.append(prediction[:, None])

                                            prev_pos = cur_pos
                                            next_ipt = action_model.embed_tokens(prediction)
                                            # import pdb
                                            # pdb.set_trace()
                            ...
                            os.makedirs(os.path.join(args.output_dir, f'preview_{global_step}step_scope_{config.prediction_scope}'), exist_ok=True)

                            # save prediction
                            with open(os.path.join(args.output_dir, f'preview_{global_step}step_scope_{config.prediction_scope}', 'action_train.json'), 'w') as f:
                        ...
                        # save prediction
                        with open(os.path.join(args.output_dir, f'preview_{global_step}step_scope_{config.prediction_scope}', 'action_val.json'), 'w') as f:
                            json.dump(action_compare, f, default=numpy_serializable)
