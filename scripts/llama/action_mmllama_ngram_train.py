...
from diffuser.models.unet_2d_condition import UNet2DConditionModel

from llama.llama_adapter_dwm import LlamaModeling
...
from nuscene_action_mmllama_ms import Actionframes
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="/mnt/storage/user/wangxiaodong/DWM_work_dir/lidar_maskgit_debug/smodels/image-ep50-s192-1e-4",
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        default="MMLlama_action_ngram_train",
        help=(
...
def main():
    ...
    # for action
    from action_mmllama_ngram_config import LlamaConfig
    config = LlamaConfig()
    ...
    with accelerator.main_process_first():
        train_dataset = Actionframes(split='train', args=config, tokenizer=tokenizer, img_size=(192, 384), history_len = config.history_len, max_video_len = config.max_video_len)
        val_dataset = Actionframes(split='val', args=config, tokenizer=tokenizer, img_size=(192, 384), max_video_len = config.max_video_len)
    ...
    if accelerator.is_main_process:
        if args.eval_steps == 1:
            args.tracker_project_name = args.tracker_project_name + '_val'
            ...
            with accelerator.accumulate(action_model):
                if not args.skip_image:
                    with torch.no_grad():
                        ...
                        # current = latents[:, 0] # (b, 4, h, w) current image can't be parallel
                        # del latents


                        # debug, default use image_0

                        # condition for current
                        text_input = batch["input_ids"]
                        encoder_hidden_states = text_encoder(text_input)[0]

                        noise = torch.randn_like(image_context)

                        if config.random_timestep:
                            timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (bs,), device=image_context.device)
                            timesteps = timesteps.long()
                        else:
                            # Sample a random timestep for each image
                            timesteps = torch.ones((bs,), device=image_context.device) * config.noise_timestep
                        
                        timesteps = timesteps.long()
                        noisy_current = noise_scheduler.add_noise(image_context, noise, timesteps)

                        outputs = unet(noisy_current, timesteps, encoder_hidden_states)

                        sample = outputs.sample
                        action_model_input_list = outputs.action_model_input_list

                        # for ts in action_model_input_list:
                        #     print(f'ts: {ts.shape}')
                        # [bsz, 640, 6, 12]
                        # [bsz, 1280, 3, 6]
                        # [bsz, 1280, 3, 6] if use mid_block feature
                        del outputs
                        # image_context = None # mmllama does not use image_context

                        if config.use_clip_embedding:
                            # TODO add clip embedding
                            # NOTE get image clip embedding
                            # NOTE first clip features
                            # vision_output = clip_model.vision_model(
                            #     batch["clip_imgs"][:, 0:1].flatten(0, 1)
                            #     .to(latents.device, dtype=clip_model.dtype))

                            # NOTE get sequence clip features
                            vision_output = clip_model.vision_model(
                                batch["clip_imgs"][:, :].flatten(0, 1)
                            .to(latents.device, dtype=clip_model.dtype))
                            ...
                            # print(f'clip embedding : {clip_embedding.shape}') # bsz, 257, 768
                            # import pdb
                            # pdb.set_trace()
                            # automatic to b t 768
                        else:
                            clip_embedding = None
                else:
                    image_context = None
                ...
                # NOTE may be bug
                # src_action = action[:, :config.history_len] # TODO cut history
                # attention_mask = attention_mask[:, :config.history_len+1]
                # TODO fix bug
                ...
                hidden_states = action_model(src_action, attention_mask, None, action_model_input_list=action_model_input_list, clip_embedding=clip_embedding)
                logits = hidden_states['logits']
                logits = logits.float()


                ngram_labels = []
                for seq_idx in range(action.shape[1]-config.n_gram + 1):
                    ngram = action[:, seq_idx: seq_idx+config.n_gram, :]
                    ngram_labels.append(ngram)
                
                ngram_labels = torch.stack(ngram_labels, dim=1)
                ngram_labels = rearrange(ngram_labels, 'b f s d -> b f (s d)') # pred n_gram per action

                # TODO fix bug
                #TODO cut ngram_labels
                # ngram_labels = ngram_labels[:, :config.history_len+1] # history hidden + 1: 1,2,3|4

                logits = logits[:, :ngram_labels.shape[1]]

                # import pdb
                # pdb.set_trace()

                shift_logits = logits
                shift_labels = ngram_labels
                loss_mask = loss_mask[:, :ngram_labels.shape[1]]

                bsz = loss_mask.shape[0]
                
                # NOTE modify for n-gram prediction
                # logits: b f 2*n-gram
                if args.loss == 'l2':
                    ...
                    loss_action = loss_action.mean(dim=-1)
                    loss_action = (loss_action.view(bsz, -1) * loss_mask).mean()
                    # NOTE the total loss, better for training
                    # 1 value
                    # loss_mask = loss_mask[:, config.history_len:config.history_len+1]
                    # loss_action = loss_action[:, config.history_len:config.history_len+1]
                    # valid_loss_len = torch.sum(loss_mask != 0, dim=1)[0].item()
                    # loss_mask = loss_mask[:, -valid_loss_len:]
                    # loss_action = loss_action[:, -valid_loss_len:]
                    # TODO only showing real loss
                ...
                if global_step % args.eval_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        noise_scheduler.set_timesteps(
                            inference_steps, device)

                        if config.eval_train_sample:
                            for n, batch in tqdm(enumerate(train_dataloader)):
                                ...
                                with torch.no_grad():
                                    ...
                                    # [0,1,2->3, 1,2,3->4]
                                    n_gram = getattr(config, 'n_gram', 1)
                                    # prediction_scope = 4 # NOTE use GT to predict how long
                                    print(f'take {config.history_len} history action, predict scope {config.prediction_scope}')
                                    # NOTE need next frame, so length -1
                                    for ac_index in tqdm(range(0, len(action[0])-config.history_len, config.prediction_scope)):
                                        ...
                                        # TODO test only
                                        sub_history_action = history_action[:, 0: ac_index+config.history_len]
                                        print(f'take {sub_history_action.shape[1]} history action')

                                        print(f'current frame idx: {ac_index+config.history_len}')
                                        # Sample a random timestep for each image
                                        # current = latents[:, ac_index+config.history_len] # pos after history 
                                        current = latents[:, 0] # TODO debug, only using current frame
                                        noise = torch.randn_like(current)

                                        # NOTE debug for diffusion loop
                                        # step_ratio = noise_scheduler.config.num_train_timesteps // inference_steps
                                        # timesteps = (np.arange(0, inference_steps) * step_ratio).round()[::-1].copy().astype(np.int64)
                                        # timesteps += noise_scheduler.config.steps_offset

                                        # NOTE denoise visulization
                                        # noisy_latents = torch.randn_like(noisy_current)
                                        # for t in range(inference_steps):
                                        #     unet_timestep = timesteps[t] # 0 -> 958
                                        #     unet_timestep = torch.tensor(unet_timestep)
                                        #     unet_timestep = unet_timestep.long()
                                        #     outputs = unet(noisy_latents, unet_timestep.repeat((bs,)).to(current.device), encoder_hidden_states)
                                        #     latent_result = outputs.sample

                                        #     latent_result = latent_result[0] # tuple[0]
                                        #     # U-Net step
                                        #     noisy_latents = noise_scheduler.step(
                                        #         latent_result, unet_timestep, noisy_latents).prev_sample
                                            
                                        #     noisy_latents_ = noisy_latents # bsz

                                        #     import pdb
                                        #     pdb.set_trace()

                                        #     image_tensor = vae.decode(
                                        #         noisy_latents_.to(dtype=vae.dtype) / vae.config.scaling_factor,
                                        #         return_dict=False)[0]
                                        #     pil_img = image_processor.postprocess(
                                        #         image_tensor, output_type="pil")
                                            
                                        #     pil_img[0].save(f'sample_denoise_t{t}_d1.png')
                                        ...
                                        # if config.random_timestep:
                                        #     timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (bs,), device=current.device)
                                        #     timesteps = timesteps.long()
                                        # else:
                                        timesteps = torch.ones((bs,), device=current.device) * config.noise_timestep
                                        timesteps = timesteps.long()
                                        noisy_current = noise_scheduler.add_noise(current, noise, timesteps)
                                        outputs = unet(noisy_current, timesteps, encoder_hidden_states)
                                        sample = outputs.sample
                                        action_model_input_list = outputs.action_model_input_list
                                        del outputs
                                        ...
                                        pred_len = 1 if config.prediction_scope==1 else math.ceil(config.prediction_scope/n_gram)
                                        total_len = prompt_len + pred_len #config.prediction_scope//n_gram # NOTE hard to set 1
                                        ...
                                        # add bos
                                        input_embed = torch.cat([action_model.bos_embed.repeat((len(input_embed), 1, 1)), input_embed], dim=1) 
                                        ...
                                        for cur_pos in range(start_pos, total_len):
                                            intermid_state = action_model(
                                                ...
                                                use_cache=True,
                                                action_model_input_list=action_model_input_list,
                                            ...
                                            if n_gram > 1: # b 1 ngram*2
                                                prediction = prediction.view(-1, n_gram, config.input_dim)
                                                prediction = prediction[:, :config.prediction_scope] # only take first prediction action
                                                pred_action.append(prediction)
                                            else:
                            ...
                            with open(os.path.join(args.output_dir, f'preview_{global_step}step', 'action_train.json'), 'w') as f:
                            ...
                            min_len = min(gather_preds.shape[1], gather_gt.shape[1])
                            gather_preds = gather_preds[:, :min_len] # cut off
                            gather_gt = gather_gt[:, :min_len]
                        ...
                        # save prediction
                        os.makedirs(os.path.join(args.output_dir, f'preview_{global_step}step'), exist_ok=True)
                        with open(os.path.join(args.output_dir, f'preview_{global_step}step', 'action_val.json'), 'w') as f:
                            json.dump(action_compare, f, default=numpy_serializable)
                        print('save done!')
                        # gather
                        gather_preds = torch.cat(gather_preds, dim=0) # tbsz, f, d
