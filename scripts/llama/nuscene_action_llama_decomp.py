    ...
    def __getitem__(self, index):
        try:
            ...
            if self.split == 'train':
                ...
                seek_start = random.randint(0, mini_len - self.args.max_video_len - self.args.history_len)
            else:
                seek_start = 0
            ...
            seek_speed_tgt = torch.tensor(seek_speed) /3.6 # maybe [0, 65]
            ...
            # we skip 8 action
            src_action_len = len(seek_steer_tgt) - 1 # leave for bos
            tgt_action_len = len(seek_steer_tgt)
            attention_mask = [1] * (1 + src_action_len) + [0] * (self.args.max_seq_len - len(seek_steer_tgt) - 1) # start with bos embed
            loss_mask = [0] * self.args.history_len + [1] * (tgt_action_len - self.args.history_len) + [0] * (self.args.max_seq_len - len(seek_steer_tgt))
            ...
            return {
                'input_ids': inputs_id,
                'label_imgs': imgs,
                'steer': seek_steer_tgt,
                'speed': seek_speed_tgt,
