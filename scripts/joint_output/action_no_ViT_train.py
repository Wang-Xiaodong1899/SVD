...
from diffuser.models.HERF_model import ActionModel
...
from nuscene_action_norm_ms import Actionframes
from nuscene_action_val_ms import Actionframes as val_Actionframes
...
def main():
    ...
    # for action
    from action_no_ViT_config import GlobalConfig
    config = GlobalConfig()
    ...
    with accelerator.main_process_first():
        train_dataset = Actionframes(split='train', args=args, tokenizer=tokenizer, img_size=(512, 512), history_len=args.history_len, max_video_len = 1)
        val_dataset = val_Actionframes(None, split='val', tokenizer=tokenizer, img_size=(512, 512), max_video_len = 8)
    ...
    # action_model.to(accelerator.device, dtype=weight_dtype)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(action_model):
                ...
                action_pred = action_model(history_action, None)

                # print(f'action_pred: {action_pred.shape}') # bsz 1 2
                # print(f'future_action: {future_action.shape}') # bsz 1 2
                
                if args.loss == 'l2':
                    loss_action = F.mse_loss(action_pred.flatten(0, 1).float(), future_action.flatten(0, 1).float(), reduction="mean")
                elif args.loss == 'l1':
                    loss_action = F.l1_loss(action_pred.flatten(0, 1).float(), future_action.flatten(0, 1).float(), reduction="mean")
                else:
                        ...
                        for n, batch in tqdm(enumerate(val_dataloader)):
                            ...
                            with torch.no_grad():
                                ...
                                for _ in range(num_frames):
                                    action_pred = action_model(history_action_, None) # b, 1, d
