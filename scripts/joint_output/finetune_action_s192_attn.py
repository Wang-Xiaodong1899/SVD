...
# no noisy to action, directly output action
# use MLP to directly predict action sequence
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # added_time_ids = torch.cat([history_action, noisy_action], 1) # set 512, receive b f 2
                # added_time_ids = torch.stack([fps, steers, speeds], dim=-1) # b, f, 3

                # Predict the noise residual and compute loss
                model_pred, action_pred = unet(noisy_latents, timesteps, encoder_hidden_states, added_time_ids, image_context=image_context, clip_embedding=clip_embedding, history_len=args.history_len, stop_action_grad=args.stop_action_grad)

                if args.snr_gamma is None:
                    loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")

                    # NOTE action loss, x-prediction
                    loss_action = F.mse_loss(action_pred.float(), future_action.flatten(0, 1).float(), reduction="mean")
