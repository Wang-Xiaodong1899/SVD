    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                elif args.prediction_type == "edm":
                    std_, mean_ = 1.4, 0.5
                    log_sigma = torch.randn((1, )) * std_ + mean_
                    sigma = torch.exp(log_sigma)
                    noise = torch.randn(latents.shape)
                    timesteps = (0.25 * log_sigma).to(latents.device, dtype=latents.dtype)
                    sigma_square_add_one = sigma ** 2 + 1
                    sqrt_sigma_square_add_one = torch.sqrt(sigma_square_add_one)

                    noisy_latents = latents + (sigma * noise).to(latents.device, dtype=latents.dtype)
                    target = (latents - noisy_latents / sigma_square_add_one.to(latents.device)) * \
                        (sqrt_sigma_square_add_one / -sigma).to(latents.device)
                    noisy_latents = noisy_latents / sqrt_sigma_square_add_one.to(latents.device, dtype=latents.dtype)
                else:
