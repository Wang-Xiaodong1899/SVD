...
class Actionframes(Dataset):
    def __init__(self, args, tokenizer: PreTrainedTokenizer, split='val', history_len = 8, max_video_len = 8, img_size=(192,384)):
        super().__init__()
    ...
    def __getitem__(self, index):
        try:
            ...
            seek_speed_ms = torch.tensor(seek_speed) / 3.6 # maybe [0, 65]

            seek_steer_tgt = torch.tensor(seek_angle) # default [-9, 9]
            seek_speed_tgt = torch.tensor(seek_speed)
