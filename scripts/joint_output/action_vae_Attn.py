    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(action_model):
                ...
                steers_tgt = batch['steer_tgt'] # b, f
                speeds_tgt = batch['speed_tgt'] # b, f


                action = torch.stack([steers, speeds], dim=-1) # b, f, 2
                action_tgt = torch.stack([steers_tgt, speeds_tgt], dim=-1) # b, f, 2

                history_action = action[:, :args.history_len]
                future_action = action_tgt[:, args.history_len:]
