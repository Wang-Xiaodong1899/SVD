...
from .resnet_action_v02 import (
    Downsample2D,
    ResnetBlock2D,
    SpatioTemporalResBlock,
    SpatioTemporalResBlockContext,
    TemporalConvLayer,
    Upsample2D,
)
from .transformer_2d import Transformer2DModel
from .transformer_temporal_v03 import (
    TransformerSpatioTemporalModel,
    TransformerTemporalModel,
)

# NOTE transformer_temporal_v03
# only spatial cross-attention differs from  v02, here only use text 77 tokens
# repeat first frame to insert after spatial resblock
# fps=2
        ...
        for i in range(num_layers):
            ...
            resnets.append(
                SpatioTemporalResBlock(
                    ...
                    eps=1e-5,
                    scale_factor=scale_factor,
                ) if i!=0 else SpatioTemporalResBlockContext(
                    in_channels=in_channels,
                    out_channels=in_channels,
                    temb_channels=temb_channels,
                    eps=1e-5,
...
class CrossAttnDownBlockSpatioTemporal(nn.Module):
    ...
    def forward(
        ...
        output_states = ()
        # encoder_hidden_states is B l 77 d
        # Suppose action is B, 2, l, d
...
class UpBlockSpatioTemporal(nn.Module):
        ...
        for i in range(num_layers):
            ...
            resnets.append(
                SpatioTemporalResBlockContext(
                    ...
                    eps=resnet_eps,
                    scale_factor=scale_factor,
                ) if i==0 and add_context else SpatioTemporalResBlock(
                    in_channels=resnet_in_channels + res_skip_channels,
                    out_channels=out_channels,
                    temb_channels=temb_channels,
                    eps=resnet_eps,
    ...
    def forward(
        ...
        for resnet in self.resnets:
            # pop res hidden states
            res_hidden_states = res_hidden_states_tuple[-1] # last feature
            res_hidden_states_tuple = res_hidden_states_tuple[:-1]
