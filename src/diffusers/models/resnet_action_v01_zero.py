...
class SpatioTemporalResBlockContext(nn.Module):
    ...
    def forward(
        ...
        if image_context is not None:
            ...
            zero_context = torch.zeros_like(image_context)
            image_context = image_context[:, None]
            
            zero_context = repeat(zero_context, 'b c h w -> b t c h w', t=num_frames-1)
            image_context = torch.cat([image_context, zero_context], dim=1)
