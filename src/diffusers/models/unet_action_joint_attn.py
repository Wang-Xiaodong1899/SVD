...
# NOTE joint
# seperately encode t_video and t_action
# joint training on fitting video and action

# Self-Attention
# [history_token, predicted_token] -> self-Attention -> [-, refined_token] -> MLP
...
class UNetSpatioTemporalConditionModel_Action(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        add_modules = [nn.Linear(time_embed_dim * (history_len), time_embed_dim)]
        for _ in range(1):
            add_modules.append(nn.GELU())
            add_modules.append(nn.Linear(time_embed_dim, (history_len) * cross_attention_dim))
        ...
        # NOTE add action prediction head
        self.action_head = nn.Linear(time_embed_dim, 2)

        self.action_to_token = nn.Linear(out_channels * 24 * 48, time_embed_dim)
        # NOTE add self-attention on history token and future token
        self.action_norm1 = nn.LayerNorm(time_embed_dim)
        self.action_attn1 = Attention(
            query_dim=time_embed_dim, # action Embedding dim
            heads=5,
            dim_head=time_embed_dim//5,
            cross_attention_dim=None,
        )
        self.action_ff1 = FeedForward(
            time_embed_dim,
            dim_out=time_embed_dim,
            mult=2,
            activation_fn="geglu",
        )
        self.action_norm2 = nn.LayerNorm(time_embed_dim)
        self.action_attn2 = Attention(
            query_dim=time_embed_dim, # action Embedding dim
            heads=5,
            dim_head=time_embed_dim//5,
            cross_attention_dim=None,
        )
        self.action_norm3 = nn.LayerNorm(time_embed_dim)
    ...
    def forward(
        ...
        added_time_ids = added_time_ids[:, :history_len]
        # added_time_ids = added_time_ids[:, history_len:]

        # print(f'added_time_ids shape: {added_time_ids.shape}')
        ...
        # NOTE history_frames
        num_frames_emb = torch.arange(history_len, device=sample.device)
        ...
        # NOTE
        # aug_emb b 2f d
        aug_emb_ = aug_emb.reshape((batch_size*(history_len), 1, -1))

        replace_embedding = aug_emb_ + real_pos_emb
        ...
        replace_embedding = rearrange(replace_embedding, 'b (f d) -> (b f) d', f=history_len)
        ...
        # NOTE action to token
        if stop_action_grad:
            sample_x = sample.reshape((batch_size, num_frames, -1)).detach()
            future_token = self.action_to_token(sample_x)
            aug_emb = aug_emb.detach()
        else:
            future_token = self.action_to_token(sample.reshape((batch_size, num_frames, -1)))

        all_action_token = torch.cat([aug_emb, future_token], dim=1) # b, 2f, d

        all_action_token = self.action_norm1(all_action_token)
        all_action_token = self.action_attn1(all_action_token, encoder_hidden_states=None) + all_action_token
        all_action_token = self.action_ff1(all_action_token) + all_action_token

        all_action_token = self.action_norm2(all_action_token)
        all_action_token = self.action_attn2(all_action_token, encoder_hidden_states=None) + all_action_token

        all_action_token = self.action_norm3(all_action_token)
        
        future_action_token = all_action_token[:, -num_frames:,]

        # NOTE action prediction
        predict_action = self.action_head(future_action_token.reshape((batch_size*num_frames, -1)))
