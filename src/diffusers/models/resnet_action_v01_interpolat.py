...
#TODO Information
# resnet_action_v01_zero
# zero means we add first frame+zero to resblock
...
class SpatioTemporalResBlockContext(nn.Module):
    ...
    def forward(
        ...
        if image_context is not None:
            # original (2b, c, h, w) outside
            image_context = rearrange(image_context, '(b l) c h w -> b l c h w', l=2)
            image_first = image_context[:, 0]
            image_last = image_context[:, 1]
            zero_context = torch.zeros_like(image_first)
            
            zero_context = repeat(zero_context, 'b c h w -> b t c h w', t=num_frames-2)
            image_first = image_first[:, None]
            image_last = image_last[:, None]

            image_context = torch.cat([image_first, zero_context, image_last], dim=1)
