...
class UNetSpatioTemporalConditionModel_Action(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        steer = added_time_ids[:, :, 0]
        speed = added_time_ids[:, :, 1]

        steer_embed = self.steer_embedding(steer.long()) # b f d
        speed_embed = self.speed_embedding(speed.long()) # b f d

        action_embed = steer_embed + speed_embed

        action_embed = action_embed.to(emb.dtype) # b f 1280

        frame_position_ids = torch.arange(num_frames, dtype=torch.long, device=emb.device)
        frame_position_ids = frame_position_ids.unsqueeze(0).expand(batch_size, -1)

        add_pos_emb = self.pos_embedding(frame_position_ids) # b f d

        action_embed = action_embed + add_pos_emb

        emb = emb + action_embed # add action into temb
        ...
        replace_embedding = action_embed

        # NOTE fix bug, need interaction
        # (bf, 1, 1280) -> (b, f*1280)
        replace_embedding = rearrange(replace_embedding, 'b f d -> b (f d)')
        ...
        clip_embedding = replace_embedding[:, None] # (b*f, 1, d)
