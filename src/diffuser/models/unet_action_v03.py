...
from .unet_3d_blocks_action_v03 import UNetMidBlockSpatioTemporal, get_down_block, get_up_block
...
class ActionEmbedding(nn.Module):
    def __init__(
        ...
        self.linear_1 = linear_cls(in_channels, embed_dim)
        ...
        if out_dim is not None:
            embed_dim_out = out_dim
        else:
            embed_dim_out = embed_dim
        self.linear_2 = linear_cls(embed_dim, embed_dim_out)
...
class UNetSpatioTemporalConditionModel_Action(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        self.action_embedder_1 = ActionEmbedding(in_channels=1, embed_dim=768*4, out_dim=768)
        self.action_embedder_2 = ActionEmbedding(in_channels=1, embed_dim=768*4, out_dim=768)
    ...
    def forward(
        ...
        steer_embedding = self.action_embedder_1(action[:, :1])
        speed_embedding = self.action_embedder_2(action[:, 1:])
        steer_embedding = rearrange(steer_embedding, '(b l) d -> b l d', b=batch_size)
        speed_embedding = rearrange(speed_embedding, '(b l) d -> b l d', b=batch_size)
        action = torch.stack([steer_embedding, speed_embedding], dim=1)
