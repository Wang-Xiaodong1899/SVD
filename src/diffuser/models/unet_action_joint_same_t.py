...
class UNetSpatioTemporalConditionModel_Action(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        add_modules = [nn.Linear(time_embed_dim * (num_frames + history_len), time_embed_dim)]
        for _ in range(1):
            add_modules.append(nn.GELU())
            add_modules.append(nn.Linear(time_embed_dim, (num_frames + history_len) * cross_attention_dim))
        ...
        self.action_head = nn.Linear(block_out_channels[0] * 24 * 48, 2)
    ...
    def forward(
        ...
        added_time_ids_his = added_time_ids[:, :history_len]
        ...
        # NOTE history
        time_embeds_his = self.add_time_proj(added_time_ids_his.flatten())
        time_embeds_his = time_embeds_his.reshape((batch_size, history_len, -1))
        time_embeds_his = time_embeds_his.to(emb.dtype)
        aug_emb_his = self.add_embedding(time_embeds_his)

        # NOTE naive adding
        aug_emb_add = aug_emb + aug_emb_his
        ...
        # NOTE history_frames + future_frames
        num_frames_emb = torch.arange(history_len + num_frames, device=sample.device)
        ...
        # NOTE concat aug_emb and aug_emb_his

        # aug_emb = rearrange(aug_emb, '(b f) d -> b f d', f=num_frames)
        # aug_emb_his = rearrange(aug_emb_his, '(b f) d -> b f d', f=history_len)

        aug_emb = torch.cat([aug_emb, aug_emb_his], dim=1) # b 2f d

        aug_emb = aug_emb.reshape((batch_size*(num_frames+history_len), 1, -1))
        ...
        replace_embedding = rearrange(replace_embedding, 'b (f d) -> (b f) d', f=num_frames+history_len)
        ...
        predict_action = self.action_head(sample.reshape((batch_size*num_frames, -1)))
