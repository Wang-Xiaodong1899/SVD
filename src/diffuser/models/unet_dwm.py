...
from .unet_3d_blocks_dwm import UNetMidBlockSpatioTemporal, get_down_block, get_up_block
from .resnet_action import Downsample2D

from einops import rearrange

# NOTE unet_dwm: driving world models
# NOTE based on unet_action_V3_0
...
class UNetSpatioTemporalConditionModel_Action(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        norm_eps: float = 1e-5, cross_attention_dim: Union[int, Tuple[int]] = 1024,
        ...
        self.proj_visual = nn.Linear(1024, time_embed_dim)
    ...
    def forward(
        ...
        action_down_pred: torch.FloatTensor = None,
        action_up_pred: torch.FloatTensor = None,
    ) -> Union[UNetSpatioTemporalConditionOutput, Tuple]:
        # NOTE action_model_output should be frame-wise, confict with clip_embedding
        ...
        # print(f't_emb: {t_emb.dtype}')

        emb = self.time_embedding(t_emb)

        emb = emb[:, None].repeat_interleave(num_frames, dim=1) #b, f, d

        # NOTE for unet_dwm, we do not need added_time_ids anymore
        # p1: only use action hidden states to guide video generation

        aug_emb = action_up_pred.to(dtype).to(emb.device) # NOTE as aug_emb
        ...
        # print(f'aug.shape {aug_emb.shape}') # b f d frame-wise
        # print(f'aug emb: {aug_emb.dtype}')
        aug_emb = self.proj_visual(aug_emb)

        emb = emb + aug_emb # NOTE aug_emb not accurate, not add here
        ...
        # print(f'add conv_in sample: {sample.shape}')
        # [bsz, 320, 24, 48]
        ...
        for idx, downsample_block in enumerate(self.down_blocks):
            ...
            if len(action_model_input_list) < 2:
                if sample.shape[1] != 320:
                    action_model_input_list.append(sample)
            # print(f'Down-block-{idx} shape: {sample.shape}')
        # [bsz, 320, 12, 24]
        # [bsz, 640, 6, 12]
        # [bsz, 1280, 3, 6]
        # [bsz, 1280, 3, 6]
        ...
        # NOTE action_up_pred need add to unet upblock
        # action_up_pred = action_up_pred.reshape((batch_size, num_frames, -1))
        # emb = emb + action_up_pred

        #NOTE use the same real_pos_emb with down-block
        # up_replace_embedding = action_up_pred + real_pos_emb

        # # NOTE fix bug, need interaction
        # # (bf, 1, 1280) -> (b, f*1280)
        # up_replace_embedding = rearrange(up_replace_embedding, '(b f) l d -> b (f l d)', b=batch_size)

        # up_replace_embedding = self.add_embedding_projector(up_replace_embedding)
        # up_replace_embedding = rearrange(up_replace_embedding, 'b (f d) -> (b f) d', f=num_frames)
        
        # up_replace_embedding = up_replace_embedding[:, None] # (b*f, l, d)
        ...
        if len(action_model_input_list) > 0:
            result = UNetSpatioTemporalConditionOutput(sample=sample, action_model_input_list=action_model_input_list)
        else:
            result = UNetSpatioTemporalConditionOutput(sample=sample)
